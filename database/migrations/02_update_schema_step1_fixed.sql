...
CREATE TABLE IF NOT EXISTS users (
    user_id UUID PRIMARY KEY REFERENCES auth.users(id),  -- Changed from 'id' to 'user_id' to match existing schema
...
CREATE TABLE IF NOT EXISTS user_stats (
    user_id UUID PRIMARY KEY REFERENCES users(user_id) ON DELETE CASCADE,  -- Updated reference to users.user_id
    total_posts_generated INTEGER DEFAULT 0,
    total_credits_used INTEGER DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS transactions (
    ...
    user_id UUID NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,  -- Updated reference to users.user_id
